...
- 문 따고 진입해서 고즈+로그 2마리 다 스폰될 때까지 프레임이 초당 한 번꼴로
  ...
  찬(=고즈+로그 2마리 스폰 완료) 시점부터 정확히 늘어나길 멈춤 — 유저가 말한
  "고즈+로그 2마리 스폰돼야 렉 풀림" 증상과 정확히 일치. 세 코루틴 모두
...
- 결과: `CamDonorSkip` 기본값을 ""로 되돌림 (`Plugin.cs`). 하이드아웃→쇄빙선
  AA 깨짐 재현 시도마다 해소 확인. 아이콘 반투명 버그는 이 변경과 무관하게
  그대로 남아있음 (별도 이슈로 분리)
