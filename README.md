...
- 2026-09-24 23:09 (KST) — 웨지방 봇 누락 추적용 **진단 로그 추가(게임 동작 변화 없음)**.
  09-24 로그 포함 실제 로그 5개를 세어 보니 5판 중 2판에서 웨지방이 4마리가 아니라
  3마리만 나왔고, 빠지는 봇이 판마다 달랐음(단독 블디 1마리 / 웨지의 경호원).
  둘 다 로그에 실패 흔적이 전혀 없었음 — 웨지방은 게임(BSG) 원래 스폰 코드로
  나오는데, 그 코드는 실패를 우리 로그 파일이 아닌 곳에 남기기 때문. 새
  `IcebreakerWedgeDiag`(웨지방 스폰을 지켜보기만 하는 클래스)가 웨지방 스폰마다
  "성공/실패, 보스 봇 정보가 비어 있었는지", "스폰 작업이 오류로 끝났는지", 그리고
  발동 25초 뒤 "예상 N마리 / 실제 M마리"를 `[WedgeDiag]`로 남김. 누락이 한 번만
  다시 나와도 T4 때처럼 서버가 봇 정보를 비워 보낸 건지 확정할 수 있음. 실제 라이드
  확인 전.
- 2026-09-24 22:07 (KST) — 주석만 정리(게임 동작 변화 없음). `IcebreakerWaveBackstop`(트리거
  박스를 놓쳤을 때 근처에서 대신 스폰시키는 보험 로직)의 설명문에 예전 값(40m)
  기준 설명이 남아 있던 것을 현재 상태(20m, 같은 층만, T3 감시 삭제)로 고치고,
  "박스를 1.5배로 넓힌 뒤로는 최후의 안전망 역할만 한다"는 현재 역할을 적어 둠.
- 2026-09-24 21:44 (KST) — T4(최상갑판 보스+경호원 5명) 스폰 코드 뒷정리. 이전
  수정(경호원 재시도)에서 "받았지만 쓸 수 없는 봇 프로필"을 목록에서 빼기만
  하고 게임에 반납하지 않던 부분을 고침 — 원본은 이런 프로필을 반납(`StopSpawn`,
  "이 봇 자리는 안 쓴다"고 게임에 알리는 기능)했는데 재시도 추가 과정에서 그
  경로가 빠졌었음. 이제 `IcebreakerFinalSquad`(T4 스쿼드를 한 번에 준비해서
  스폰하는 클래스)가 쓸 수 없는 경호원 프로필과, 새 리더로 교체된 원래 리더
  프로필을 반납함. 리더 재시도까지 실패한 경우엔 원본과 똑같이 게임이 원래
  준 리더로 나중에 다시 시도하게 되돌려 둠(재시도 결과가 비어 있으면 그
  재시도 경로 자체가 사라지던 문제도 같이 해결). 정상적으로 한 번에 뜨는
  판에서는 동작 변화 없음. 실제 라이드 확인 전.
- 2026-09-20 19:58 — 네 번째 실제 라이드 로그로 최종 확인: 엔진룸/헬리패드/
  웨지(T1~T3)는 백스톱 없이 원본 트리거 박스만으로 정상 발동, T4도 리더
  프로필 재시도가 딱 한 번 걸리고 그 즉시 5/5 스폰 성공(바로 아래 두 항목의
  "아직 확인 전" 상태였던 수정이 실제로 확인됨 — 26초/5연속 실패하던 것이
  사실상 즉시 해결). 이번 세션에서 다룬 트리거/스폰 이슈는 여기서 종료.
  별도로 웨지방(블디+웨지 4마리 조합) 봇이 가끔 하나 누락된다는 제보가
  있었으나 아직 로그로 확인 안 됨 — 다음에 재현 로그 받으면 이어서 확인.
- 2026-09-20 18:30 — 바로 위 수정(T4 리더 재시도) 덕분에 T4가 이제 뜨긴 뜨는데,
  실제 로그를 보니 트리거되고 **26초 동안 5번 연속 실패**하다 마지막에야
  성공해서, 그 사이 플레이어가 이미 방을 지나 나가는 쪽까지 이동한 뒤에
  스쿼드가 뒤에서 등장하는 문제 확인(원작에도 있던 "숨을 곳 못 찾으면 계속
  재시도" 문제 — README 09/07 참고, 리더 문제 때문에 여태 안 보였을 뿐). 실패
  5번 중 3번이 "경호원 4명 중 프로필을 못 받아온 것"이었어서, 리더와 같은
  방식으로 경호원 쪽도 비어있는 자리만 한 번 더 요청하도록 재시도 추가.
  (남은 2번의 실패는 "숨을 곳 부족"이라 별개 원인 — 이번 수정 대상 아님.)
  **19:58 항목에서 실제로 확인됨 — 재시도 1회로 즉시 성공.**
- 2026-09-20 18:05 — 최상갑판(T4) 보스+경호원 5명 스쿼드가 아예 안 뜨던 문제에
  방어 로직 추가. 로그에서 `T4 leader must contain one valid profile`(보스
  봇 데이터를 서버에서 받아오는 과정이 실패해서 정보가 비어있음) 에러가 뜨면
  스쿼드 전체가 포기되고 다시 시도하지 않았음 — 이건 원작(danauraborealis)에도
  그대로 있는 원본 코드라 저희가 새로 만든 버그는 아니지만, 실제로 막히는
  걸 확인했으니 한 번 더 요청해서 받아오도록 재시도 로직을 추가함(그래도
  실패하면 기존처럼 포기). **19:58 항목에서 실제로 확인됨** — 다만 원인
  자체가 서버와 통신하다 생기는 것이라 다음에도 100% 안 뜬다는 보장은 없음.
